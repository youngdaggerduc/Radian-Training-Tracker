...
// Stages used for pipeline ordering
...
function getCourse(id) { return COURSES.find(c => c.id === id) || { name: "—", price: 0, days: 0, provider: "" }; }
function getStaff(id) { return STAFF.find(s => s.id === id) || { name: "—", initials: "?" }; }
function getInstructor(id) { return INSTRUCTORS.find(i => i.id === id) || { name: "—" }; }

window.RADIAN = {
  COURSES, STAFF, INSTRUCTORS, LOCATIONS, PIPELINE_STAGES,
  todayISO, fmtDate, fmtDateShort, fmtMoney, daysBetween, daysUntil, relTime,
  buildSeed, loadState, saveState, resetState,
  leadStage, traineeStage, getCourse, getStaff, getInstructor,
};
})();
