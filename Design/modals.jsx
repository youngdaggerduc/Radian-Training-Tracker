// Shared modal forms: Add Lead, Schedule Follow-up, Record Payment, Enroll Student, Issue Certificate
const { COURSES, STAFF, INSTRUCTORS, LOCATIONS, fmtDate, fmtMoney, todayISO, getCourse, getStaff, getInstructor } = window.RADIAN;
...
function ConvertToPaymentModal({ lead, onClose, onConvert }) {
  ...
  const [f, setF] = React.useState({
    ...
    firstDueOffset: 30, // first installment after training (in days from today)
    notes: "",
  });
  const set = (k, v) => setF(s => ({ ...s, [k]: v }));
...
function EditPlanModal({ trainee, onClose, onSave }) {
  // Split into paid (locked) and unpaid (editable)
  ...
  const [unpaid, setUnpaid] = React.useState(initialUnpaid.length > 0 ? initialUnpaid : [
...
function RecordPaymentModal({ trainee, installment, onClose, onRecord }) {
  const [f, setF] = React.useState({
...
// ============ ENROLL STUDENT ============
function EnrollModal({ trainee, onClose, onEnroll }) {
...
// ============ ISSUE CERTIFICATE ============
function CertificateModal({ trainee, onClose, onIssue }) {
  const course = getCourse(trainee.courseId);
  const [f, setF] = React.useState({
...
// ============ COLLECT CERTIFICATE ============
function CollectCertModal({ trainee, onClose, onCollect }) {
  const [f, setF] = React.useState({
...
window.RadianModals = { Modal, AddLeadModal, FollowUpModal, ConvertToPaymentModal, RecordPaymentModal, EditPlanModal, EnrollModal, CertificateModal, CollectCertModal };
