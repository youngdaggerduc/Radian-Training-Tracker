// Dashboard with stat cards, reminders, quick actions
const { fmtDate: fD, fmtDateShort: fDS, fmtMoney: fM, todayISO: tISO, daysUntil: dU, relTime: rT, getCourse: gC, getStaff: gS, traineeStage: tStage, leadStage: lStage } = window.RADIAN;

function Dashboard({ state, setView, openModal, openDrawer, currentUser }) {
  ...
  // Important reminders (1 month advance): trainings starting in <= 30 days but no enrollment
  const importantReminders = [];

  // Overdue payments (most urgent)
  ...
  // Upcoming payments due (within 30 days = 1 month advance)
  ...
  // Trainings starting in <=30 days (1 month advance reminder)
      ...
      <div className="two-col">
        ...
        <div className="panel">
          ...
          <div style={{padding:"0 18px 18px"}}>
            ...
            <div className="kv"><span className="k">Active staff</span><span className="v">{window.RADIAN.STAFF.length}</span></div>
          </div>
        </div>
      </div>
...
function StatCard({ label, value, sub, onClick, accent, alert }) {
  return (
