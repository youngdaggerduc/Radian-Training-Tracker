...
// ============ PAYMENTS VIEW ============
export function PaymentsView({ state, openDrawer, openModal }) {
  ...
  const matchesSearch = (t) => {
    ...
    return [t.name, t.company, t.phone, t.email, t.id, RD.getCourse(t.courseId).name]
  ...
  const overdueAmount = state.trainees.reduce((s, t) => s + t.plan.filter(p => !p.paid && RD.daysUntil(p.due) < 0).reduce((a, p) => a + p.amount, 0), 0);
  const upcoming30 = state.trainees.reduce((s, t) => s + t.plan.filter(p => !p.paid && RD.daysUntil(p.due) >= 0 && RD.daysUntil(p.due) <= 30).reduce((a, p) => a + p.amount, 0), 0);
        ...
        <table className="tbl">
          ...
          <tbody>
            {list.length === 0 ? (
              ...
              const overdue = next && RD.daysUntil(next.due) < 0;
              ...
              return (
                <tr key={t.id} onClick={() => openDrawer({ type: "trainee", id: t.id })}>
                  ...
                  <td><span className="tnum">{RD.fmtMoney(t.paid)}</span></td>
                  <td><span className="tnum" style={{color: t.totalCost - t.paid > 0 ? "var(--orange-dk)" : "var(--green)"}}>{RD.fmtMoney(t.totalCost - t.paid)}</span></td>
                  <td>{next ? <div><div className="nm tnum">{RD.fmtMoney(next.amount)}</div><div className="sub">{RD.fmtDateShort(next.due)} · {RD.relTime(next.due)}</div></div> : <span style={{color:"var(--green)"}}>Complete</span>}</td>
                  <td>
...
// ============ ENROLLMENT VIEW ============
export function EnrollmentView({ state, openDrawer, openModal }) {
  ...
  const matchesSearch = (t) => {
    if (!search.trim()) return true;
    const q = search.toLowerCase();
    return [t.name, t.company, t.phone, t.email, t.id, RD.getCourse(t.courseId).name, t.enrollment?.batch, t.enrollment?.location]
      .filter(Boolean).some(v => String(v).toLowerCase().includes(q));
  };
              ...
              return (
                <tr key={t.id} onClick={() => openDrawer({ type: "trainee", id: t.id })}>
                  ...
                  {tab === "ready" ? (
                    <>
                      <td>
                        <div className="tnum">{RD.fmtMoney(t.paid)} / {RD.fmtMoney(t.totalCost)}</div>
                    ...
                    <>
                      <td>
                        <div className="nm">{RD.fmtDateShort(e.startDate)} – {RD.fmtDateShort(e.endDate)}</div>
                        <div className="sub">{e.location}</div>
                      </td>
                      <td>
                        <div className="nm">{RD.getInstructor(e.instructor).name}</div>
...
// ============ CERTIFICATES VIEW ============
export function CertificatesView({ state, openDrawer, openModal }) {
            ...
            {list.length === 0 ? (
              ...
              return (
                <tr key={t.id} onClick={() => openDrawer({ type: "trainee", id: t.id })}>
                  ...
                  <td>{RD.fmtDate(t.enrollment?.endDate || t.certificate?.completionDate)}</td>
                  ...
                  <td>
                    ...
                    {tab === "collected" && (
                      <div className="sub">Collected {RD.fmtDateShort(t.certificate?.collectionDate)}</div>
                    )}
                  </td>
                </tr>
              );
    ...
    <>
      ...
      <div className="drawer">
        ...
        <div className="drawer-body">
            ...
            <>
              <div style={{padding:"18px 20px", background:"var(--navy-800)", color:"var(--ivory)", borderRadius:"var(--radius-lg)", marginBottom:24}}>
                ...
                <div style={{display:"flex", justifyContent:"space-between", marginTop:8, fontSize:12}}>
                  <span style={{opacity:0.65}}>Paid <span className="tnum">{RD.fmtMoney(trainee.paid)}</span></span>
                  <span style={{color:"var(--orange)", fontWeight:500}} className="tnum">{remaining > 0 ? RD.fmtMoney(remaining) + " remaining" : "Paid in full"}</span>
                </div>
              </div>

              <h3 style={{fontSize:14, marginBottom:12, color:"var(--navy-500)"}}>Trainee Journey</h3>
              <PipelineProgress stage={RD.traineeStage(trainee)}/>
          ...
          {tab === "payments" && (
            <>
              <div className="kv"><span className="k">Course Cost</span><span className="v tnum">{RD.fmtMoney(trainee.totalCost)}</span></div>
              <div className="kv"><span className="k">Total Paid</span><span className="v tnum" style={{color:"var(--green)"}}>{RD.fmtMoney(trainee.paid)}</span></div>
              <div className="kv"><span className="k">Remaining Balance</span><span className="v tnum" style={{color: remaining > 0 ? "var(--orange-dk)" : "var(--green)"}}>{RD.fmtMoney(remaining)}</span></div>
                  ...
                  <div key={p.id} className="note-item" style={{borderLeftColor: p.paid ? "var(--green)" : (overdue ? "var(--red)" : "var(--orange)"), background:"var(--paper)", border:"1px solid var(--navy-100)", borderLeftWidth:3, marginBottom:10}}>
                    <div style={{display:"flex", justifyContent:"space-between", alignItems:"flex-start"}}>
                      <div>
                        ...
                        <div style={{fontSize:18, fontFamily:"var(--serif)"}} className="tnum">{RD.fmtMoney(p.amount)}</div>
                        <div className="meta" style={{fontSize:12, color:"var(--navy-500)", marginTop:4}}>Due {RD.fmtDate(p.due)} {!p.paid && <span style={{color: overdue ? "var(--red)" : "var(--navy-500)"}}>· {RD.relTime(p.due)}</span>}</div>
                      </div>
                      {p.paid ? (
                        <div style={{textAlign:"right"}}>
                          <span className="pill paid"><Icon name="check" size={10}/> Paid {RD.fmtDateShort(p.paidOn)}</span>
            ...
            <>
              {trainee.enrollment ? (
                <>
                  ...
                  <div className="kv"><span className="k">Start Date</span><span className="v">{RD.fmtDate(trainee.enrollment.startDate)}</span></div>
                  <div className="kv"><span className="k">End Date</span><span className="v">{RD.fmtDate(trainee.enrollment.endDate)}</span></div>
                  <div className="kv"><span className="k">Location</span><span className="v">{trainee.enrollment.location}</span></div>
                  <div className="kv"><span className="k">Instructor</span><span className="v">{RD.getInstructor(trainee.enrollment.instructor).name}</span></div>
                  <div className="kv"><span className="k">Batch Code</span><span className="v mono">{trainee.enrollment.batch}</span></div>
                  <div className="kv"><span className="k">Enrolled On</span><span className="v">{RD.fmtDate(trainee.enrollment.enrollmentDate)}</span></div>
            ...
            <>
              {trainee.certificate ? (
                <>
                  ...
                  <div className="kv"><span className="k">Completion Date</span><span className="v">{RD.fmtDate(trainee.certificate.completionDate)}</span></div>
                  <div className="kv"><span className="k">Status</span><span className="v"><StatusPill status={trainee.certificate.status}/></span></div>
                  <div className="kv"><span className="k">Ready On</span><span className="v">{RD.fmtDate(trainee.certificate.readyOn)}</span></div>
                  {trainee.certificate.collectionDate && (
                    <>
                      <div className="kv"><span className="k">Collected On</span><span className="v">{RD.fmtDate(trainee.certificate.collectionDate)}</span></div>
